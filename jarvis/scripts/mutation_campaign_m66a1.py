...
M = [
    ...
    # ── CREDENTIAL (Round-2: cross-origin header handling is now an ALLOWLIST) ──
    ("cred_crossorigin_keeps_all", "tools/executor.py",
     "        if k.lower() in _CROSS_ORIGIN_SAFE_HEADERS:", "        if True:",
     [f"{_T}::TestF3Credentials::test_cross_origin_strips_each_sensitive_header",
      f"{_T}::TestRound2Fixes::test_f3_cross_origin_allowlist_only_keeps_safe"]),
    ("cred_allowlist_admits_authorization", "tools/executor.py",
    ...
    ("cred_allowlist_admits_cookie", "tools/executor.py",
     "_CROSS_ORIGIN_SAFE_HEADERS: frozenset[str] = frozenset({\n    \"user-agent\",",
     "_CROSS_ORIGIN_SAFE_HEADERS: frozenset[str] = frozenset({\n    \"cookie\", \"user-agent\",",
     [f"{_T}::TestF3Credentials::test_cross_origin_strips_each_sensitive_header[Cookie]"]),
    ("cred_allowlist_admits_private_token", "tools/executor.py",
     "_CROSS_ORIGIN_SAFE_HEADERS: frozenset[str] = frozenset({\n    \"user-agent\",",
     "_CROSS_ORIGIN_SAFE_HEADERS: frozenset[str] = frozenset({\n    \"private-token\", \"user-agent\",",
     [f"{_T}::TestRound2Fixes::test_f3_custom_credential_header_stripped_crossorigin[PRIVATE-TOKEN]"]),
    ...
    # ── ROUND-2 REMEDIATION (load-bearing checks for the F1/F3/F4 fixes) ──────
    ("cred_auth_header_not_scrubbed", "core/tool_approval.py",
     "    scrubbed = _AUTH_HEADER_RE.sub(\n        lambda m: f\"{m.group(1)}{m.group(2)}{m.group(3)}{m.group(4)}<redacted>\", scrubbed)",
     "    scrubbed = scrubbed  # mut: auth-header scrub removed",
     [f"{_T}::TestRound2Fixes::test_f2_authorization_basic_in_command_redacted"]),
    ("cred_nested_redaction_disabled", "core/tool_approval.py",
     "            if isinstance(k, str) and _is_sensitive(k):",
     "            if False:",
     [f"{_T}::TestRound2Fixes::test_f2_headers_dict_authorization_redacted"]),
    ("ingest_per_file_containment_removed", "core/knowledge.py",
     "            if match.resolve().is_relative_to(folder_real):",
     "            if True:",
     [f"{_T}::TestRound2Fixes::test_ingest_contained_matches_drops_symlink_out"]),
