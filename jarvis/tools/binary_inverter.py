"""
tools/binary_inverter.py — Automated Binary Inversion & Triage Engine (v20.0).

_re_pool is a dedicated ProcessPoolExecutor(max_workers=1).
Do NOT share with _mesh_pool, _graph_pool, or _vol_pool.

Worker function _reverse_engineer_worker must be module-level (not a lambda or
local function) for Windows multiprocessing pickling compatibility.
...
# Dangerous Windows APIs commonly abused by malware
...
# Pattern matching suspicious recovered strings
_SUSPICIOUS_STR_RE = re.compile(
    r"(?i)("
    r"https?://"
    r"|cmd\.exe|powershell\.exe|wscript\.exe|cscript\.exe|mshta\.exe"
    r"|\\\\[A-Za-z0-9_.-]+\\[A-Za-z0-9_$]"   # UNC path
    r"|HKEY_(LOCAL_MACHINE|CURRENT_USER|CLASSES_ROOT)"
    r"|\\Run\\|\\RunOnce\\"
    r"|meterpreter|beacon|shellcode|payload"
    r"|base64_decode|FromBase64|[A-Za-z0-9+/]{40,}={0,2}"  # long base64
    r"|\\x[0-9a-fA-F]{2}(\\x[0-9a-fA-F]{2}){7,}"         # hex shellcode
    r")"
)

# Printable ASCII string extractor (>= 6 chars)
...
def _reverse_engineer_worker(file_path: str) -> dict:
    ...
    1. PE parsing with pefile:
       - Full IAT extraction
       - Per-section Shannon entropy (packed if any section > 7.0 bits)
       - Overlay detection (appended data after last section)

    2. Capstone x86-64 disassembly of .text section:
       - XOR loop detection (decryption key candidates)
       - GetProcAddress chain detection (dynamic API rebuild)
       - CALL/JMP-to-register patterns (shellcode stub)

    3. Raw ASCII string extraction:
       - Strings >= 6 chars
       - Filtered against _SUSPICIOUS_STR_RE

    Returns plain dict only — no pefile/capstone objects (not picklable).
    """
    import gc
    ...
    try:
        ...
        if text_section is not None:
            ...
            gpa_chain     = 0
            jmp_reg_count = 0
            limit         = 5000  # cap disassembly to avoid long runtime
            ...
            for insn in cs.disasm(text_data, text_rva):
                ...
                if insn.id == capstone.x86.X86_INS_XOR:
                    ...
                    if len(ops) == 2 and ops[0].type != ops[1].type:
                        xor_count += 1   # XOR reg, imm/mem — likely decryption
                ...
                if insn.id in (capstone.x86.X86_INS_JMP, capstone.x86.X86_INS_CALL):
                    ...
                    if ops and ops[0].type == capstone.x86.X86_OP_REG:
                        jmp_reg_count += 1   # JMP/CALL reg — indirect dispatch (shellcode)

            notes: list[str] = []
            if xor_count > 8:
                notes.append(f"XOR decryption loop detected ({xor_count} XOR reg,imm instructions)")
            if gpa_chain > 2:
                notes.append(f"Dynamic API rebuild via GetProcAddress chain ({gpa_chain} calls)")
            if jmp_reg_count > 15:
    ...
    # ── 3. String extraction ──────────────────────────────────────────────────
    try:
        candidates = [s.decode("ascii", errors="replace")
                      for s in _ASCII_STR_RE.findall(raw)]
        suspicious = [s for s in candidates if _SUSPICIOUS_STR_RE.search(s)]
        result["recovered_strings"] = suspicious[:40]  # cap at 40 strings
    except Exception as exc:
...
async def execute_automated_triage(file_path: str, broadcast_fn) -> None:
    ...
    try:
        ...
        await broadcast_fn({"type": "error", "error": f"Binary triage failed: {exc}"})
