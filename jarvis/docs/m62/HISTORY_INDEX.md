...
> **Gap, recorded rather than papered over.** **S3R**, **S3S**, **S3S.1** and **S3T.0** each have a milestone document in `jarvis/docs/` and no row above; S3U did not write rows for milestones it did not run. Find them by filename, or through the generation they wrote in `state/m62/snapshots/`.
